...
function wireless_load_content()
{
	...
	//var auth = "Basic " + base64.encode('admin:');
		$.ajax({
			type:"GET",
			url:dvr_url + "/netsdk/Network/Interface/4/Wireless_B64En",
			dataType:"json",
			beforeSend : function(req){ 
				req .setRequestHeader('Authorization', auth);
				},
			success:function(data){
				...
				if(aa == 'none')
				{
					...
					$('#wirelessApEssId_sta').val(base64.decode(data.stationMode.wirelessApEssId));
					$('#wirelessApPsk_sta').val(base64.decode(data.stationMode.wirelessApPsk));
					$('#wirelessStaMode_sta').val(data.stationMode.wirelessStaMode);
					//accessPointMode
					$('#wirelessApBssId_acc').val(data.accessPointMode.wirelessBssId);
					$('#wirelessApEssId_acc').val(base64.decode(data.accessPointMode.wirelessEssId));
					$('#wirelessApPsk_acc').val(base64.decode(data.accessPointMode.wirelessPsk));
