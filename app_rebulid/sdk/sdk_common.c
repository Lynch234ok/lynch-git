...
int isp_ircut_switch_hardware_control_lightmode(int gpio_status_cur, lpstSwitchIrcutInfo md_info)
{
	...
	if(count++ >= DAY_NIGHT_CHANGER_DELAY){
		switch(md_info->daynight_mode_state){
			...
			break;
			case STATE_NIGHT:
				gain_current = md_info->cur_expose_factor;
				if(0 == gain_last){
					gain_last = gain_current;
					break;
				}
				if(md_info->cur_expose_factor < 2){
					if(ircut_edge_detect2(&state_tmp)){
							_isp_ir_switch->isp_white_light_switch(0);//night->light_on
							md_info->daynight_mode_state = STATE_DAY;
							count = 0;
					}
				}else{
					if(gain_last*0.8 > gain_current){
						if(ircut_edge_detect2(&state_tmp)){
							_isp_ir_switch->isp_white_light_switch(0);//night->light_on
							md_info->daynight_mode_state = STATE_DAY;
							count = 0;
