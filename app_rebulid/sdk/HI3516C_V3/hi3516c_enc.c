...
static int enc_create_stream_h264(int vin, int stream, LP_SDK_ENC_STREAM_H264_ATTR h264_attr)
{
	...
	//		VENC_PARAM_H264_CBR_S h264_cbrv2;
			...
			p_venc_attr_h264->u32BufSize  = p_venc_attr_h264->u32MaxPicWidth * p_venc_attr_h264->u32MaxPicHeight  * 2 / 5; // stream buffer size
...
//			p_venc_attr_h264->u32RefNum = 1;/* 0: default; number of refrence frame*/
	...
	//					vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
						p_h264_avbr->u32Gop =  streamH264Attr->fps*2;
...
*/
	...
	//							stVencRcPara.stParamH264AVbr.enQpMapMode = VENC_RC_QPMAP_MODE_MEANQP; 
								...
								stVencRcPara.stParamH264AVbr.s32MinStillPercent = 5;
...
static int enc_create_stream_h265(int vin, int stream, LP_SDK_ENC_STREAM_H265_ATTR h265_attr)
{
		...
		//		VENC_PARAM_H264_CBR_S h264_cbrv2;
				...
				p_venc_attr_h265->u32BufSize  = p_venc_attr_h265->u32MaxPicWidth * p_venc_attr_h265->u32MaxPicHeight * 2 / 5;//2 // stream buffer size
				...
				switch(h265_attr->rc_mode){
					...
					case kSDK_ENC_H265_RC_MODE_VBR://FIX ME !!!!!
					{
						if(h265_attr->_enable_smartP){
							...
							vencChannelAttr.stGopAttr.stSmartP.s32ViQpDelta = 2;
	//						vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
							p_h265_avbr->u32Gop =  streamH265Attr->fps*2;
						...
						case VENC_RC_MODE_H265AVBR:
							{							
								...
								if(h265_attr->_enable_smartP){
									...
									stVencRcPara.stParamH265AVbr.s32MinStillPercent = 5;
			...
			if((strcmp(streamH264Attr->name ,h264_attr->name) == 0 ) && (streamH264Attr->width == h264_attr->width) &&  (streamH264Attr->height == h264_attr->height)  &&  (streamH264Attr->profile == h264_attr->profile) 
				...
				if (s32Ret == HI_SUCCESS){
					if(VENC_RC_MODE_H264CBR == venc_ch_attr.stRcAttr.enRcMode){
						 ...
						 venc_ch_attr.stRcAttr.stAttrH264AVbr.u32Gop = h264_attr->fps * 2;
				...
				if((strcmp(streamH265Attr->name ,h265_attr->name) == 0 ) && (streamH265Attr->width == h265_attr->width) &&	(streamH265Attr->height == h265_attr->height)  &&  (streamH265Attr->profile == h265_attr->profile) 
					...
					if (s32Ret == HI_SUCCESS){
						if(VENC_RC_MODE_H265CBR == venc_ch_attr.stRcAttr.enRcMode){
							 ...
							 venc_ch_attr.stRcAttr.stAttrH265AVbr.u32Gop = h265_attr->fps * 2;
			...
			if(venc_chn_stat.u32CurPacks > 0){				
				...
				//		is_keyframe = (H264E_NALU_ISLICE == venc_pack[venc_chn_stat.u32CurPacks-1].DataType.enH264EType);
						...
						if(1){//if(0 == streamH264Attr->ref_counter || 2 == streamH264Attr->ref_counter){
                            ...
                            attr.soupFrameHead.magic = 0x534f55ff;
                            attr.soupFrameHead.version = 0x01000000;
                            attr.soupFrameHead.framesize = attr.data_sz;
                            attr.soupFrameHead.frametype = is_keyframe?1:2;
                            attr.soupFrameHead.ts_sec = attr.time_us/1000000;
                            attr.soupFrameHead.ts_usec = attr.time_us%1000000;
                            attr.soupFrameHead.externsize = 0;
                            memcpy(&attr.soupFrameHead.v.enc,"H264",strlen("H264"));
                            attr.soupFrameHead.v.height = streamH264Attr->height;
                            attr.soupFrameHead.v.width = streamH264Attr->width;
					...
					case kSDK_ENC_BUF_DATA_H265:
						...
						if(1){
                            ...
                            attr.soupFrameHead.ts_usec = attr.time_us%1000000;
                            attr.soupFrameHead.externsize = 0;
                            memcpy(&attr.soupFrameHead.v.enc,"H265",strlen("H265"));
                            attr.soupFrameHead.v.height = streamH264Attr->height;
                            attr.soupFrameHead.v.width = streamH264Attr->width;
				
							if(0 == do_buffer_request(streamH265Attr->buf_id, attr.data_sz + sizeof(attr), attr.h265.keyframe)){	
								int i = 0;
								// buffer in the attribute
...
static int enc_audio_proc(int ain)
{
	...
	if(0 != stream_g711_attr->packet_size){
		...
		if(HI_SUCCESS == HI_MPI_AENC_GetStream(ain, &audio_stream, HI_IO_NOBLOCK)){
            ...
            attr.soupFrameHead.version = 0x01000000;
            attr.soupFrameHead.framesize = attr.data_sz;
            attr.soupFrameHead.frametype = 0;//audio
            attr.soupFrameHead.ts_sec = attr.time_us/1000000;
            attr.soupFrameHead.ts_usec = attr.time_us%1000000;
            attr.soupFrameHead.externsize = 0;
            if(audio_type == AENC_CODEC_TYPE_AAC){
                snprintf((char *)&attr.soupFrameHead.a.enc, sizeof(attr.soupFrameHead.a.enc), "%s", "AAC");
            }else if(audio_type == AENC_CODEC_TYPE_G711A){
                snprintf((char *)&attr.soupFrameHead.a.enc, sizeof(attr.soupFrameHead.a.enc), "%s", "G711");
            }
            attr.soupFrameHead.a.samplerate = 8000;
            attr.soupFrameHead.a.samplewidth = 16;
		...
		for(i = 0; i < HI_VENC_OVERLAY_BACKLOG_REF; ++i){
			...
			if(!overlay->canvas){
				...
				if(1){//if(overlay->region_handle >= 0){
					...
					region_attr.unAttr.stOverlay.u32CanvasNum = 2;
					...
					switch(vencChannelAttr.stRcAttr.enRcMode){
						...
						case VENC_RC_MODE_H264AVBR:
							...
							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate ){
								...
								vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32StatTime * vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate;
							//	SOC_INFO("u32Gop=%d u32StatTime=%d",vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop,vencChannelAttr.stRcAttr.stAttrH264AVbr.u32StatTime);
								...
								vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32StatTime * actual_expos_fps;
						...
						case VENC_RC_MODE_H265AVBR:
							...
							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate ){
								...
								vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32StatTime * vencChannelAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate;
							//	SOC_INFO("u32Gop=%d u32StatTime=%d",vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop,vencChannelAttr.stRcAttr.stAttrH265AVbr.u32StatTime);
								...
								vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32StatTime * actual_expos_fps;
...
static int enc_slow_framerate_syn_start()
{
	if(!_sdk_enc.attr.slow_framerate_tid){
		...
		ret = pthread_create(&_sdk_enc.attr.slow_framerate_tid, NULL, (void *)hi3516e_enc_set_slow_framerate_syn_loop, NULL);
		return ret;
	}
	return -1;
}
