...
void MegCheck(char driverName[200], char interfaceName[100],char CtrlInterface[200],char  ConfigueFile[200])
{
	...
	sprintf(str, "/usr/share/ipcam/wifi_tools/wpa_supplicant -D%s -i%s -C%s -c%s -B &", driverName, interfaceName, CtrlInterface, ConfigueFile);
	NK_SYSTEM(str);
	/*fb= popen(str, "r");
    if(fb==NULL)
	{ 
...
void STA_passphraseSTA(char essid[100], char passwd[100], char ConfigueFile[200])
{
	...
	sprintf(str, "/usr/share/ipcam/wifi_tools/wpa_passphrase '%s' '%s' > %s", essid, passwd, ConfigueFile);
	NK_SYSTEM(str);
    /*fp= popen(str, "r");
    if(fp==NULL)
	{ 
