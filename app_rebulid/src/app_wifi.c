...
{
	system("ifconfig wlan0 down;ifconfig wlan1 down");
	system("rmmod rtl8188eus;rmmod rtl8188fu"); //for hisi
	system("rmmod 8188eu;rmmod 8188fu");        //for ak
	sleep(1);
...
fWIFI_SYSTEM_REBOOT_do APP_WIFI_self_reboot()
{
	return wifi_self_reboot;
...
static int usb1_wifi_mod_get_idProduct(char *_idProduct, int _size)
{
	...
	fp = popen("lsusb", "r");		//popen作用：shell 将执行"lsusb"这个命令
	if(fp == NULL){ 
		perror("[popen] lsusb");
	}
	else{
		while(fgets(buf, sizeof(buf), fp)){		//fget：从文件结构体指针fp中读取数据，每次读取一行。读取的数据保存在buf指向的字符数组中
			if(strstr(buf, "ID")){
...
fWIFI_SYSTEM_CHECK_SMARTLINK_do APP_WIFI_check_smartlink_status()
{
#if defined(WIFI)
...
{	
	...
	if(strcmp(wifi_stat.wifi_chip, "rtl8188eu") && strcmp(wifi_stat.wifi_chip, "rtl8821au") && strcmp(wifi_stat.wifi_chip, "rtl8188fu")){
			...
			if(!strcmp(idProduct, "8176") || !strcmp(idProduct, "8179")){
        		...
        		system("insmod /usr/share/ipcam/mpp/juan_ko/rtl8188eus.ko");
				char map[8] = {0};
        		...
        		system("insmod /usr/share/ipcam/mpp/juan_ko/rtl8811AU.ko");
        	}else if(!strcmp(idProduct, "f179") ){
        		ret_val = true;
				model_type = 1;
        		strcpy(wifi_stat.wifi_chip, "rtl8188fu");
        		system("insmod /usr/share/ipcam/mpp/juan_ko/rtl8188fu.ko");
...
int APP_WIFI_Wifi_Exit()
{
	...
	JN_Wifi_Exit();
	//��concurrentģʽ�����õ�AP����Ϊmanagedģʽ,��ֹAP�ز��������
	/*
