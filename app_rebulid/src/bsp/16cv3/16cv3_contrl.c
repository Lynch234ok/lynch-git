...
static int sound_input_gain_level = 0;
static void v2_GPIO_Init(void)
{
	uint32_t reg_val = 0;
		...
		BSP_gpio_pin_write(WHITE_LED_GPIO_GROUP, WHITE_LED_GPIO_PIN, 0);//WHITE LED off
...
static void v2_Speaker_Enable(bool Enable)
{
	uint32_t reg_val = 0;
		...
		reg_val &= ~(0x1f << SOUND_PLUS_CTRL_BIT);	// «Â¡„
...
/* 
    ...
    ≤Œ ˝¥´»Î 0 ±Ì ææÕº”√‹∆¨
*/
void BSP_ContrlInit(int val, int audioHwSpec, int model_name)
{
