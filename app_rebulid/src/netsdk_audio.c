				...
				if(0 == id){
					...
					if(NULL != channel){
						...
						if(netsdk->audioInputChannelChanged){
							...
							if(NETSDK_conf_ain_ch_get(id, &ain_ch)){
								netsdk->audioInputChannelChanged(id, &ain_ch);
							}
						}
...
static LP_NSDK_AIN_CH netsdk_conf_ain_ch(bool set, int id, LP_NSDK_AIN_CH ain_ch)
{
	char text[128] = {""};
