...
static LP_MODEL_CONF model_conf_json_parse(LP_JSON_OBJECT json, LP_MODEL_CONF model_conf)
{
	...
	if(video_json){
		video_channel_main = model_conf_video_find_channel(video_json, MODEL_CONF_VIDEO_MAIN_STREAM_ID);
		video_channel_sub = model_conf_video_find_channel(video_json, MODEL_CONF_VIDEO_SUB_STREAM_ID);
		//main stream
...
static void model_conf_make_file_path(emSENSOR_MODEL sensor_type, char *file_path)
{
    ...
    /* 根据product type确定model_conf路径 */
    /* 没有读到product type，则默认 MODEL_CONF_FILE_DIR/Px */
