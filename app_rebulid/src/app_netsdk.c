...
static int netsdk_system_changed(LP_NSDK_SYSTEM_TIME sys_time)
{
	...
	if(sys_time->ntpEnabled){
		NTP_start(sys_time->ntpServerDomain, NULL, 5, 0); // ÒòÎª×îÐÂµÄÐÞ¸Ä£¬ntpÍ¬²½Ê±²»ÉèÖÃÊ±Çø£¬ËùÒÔ²ÎÊý4²»ÉúÐ§
	}
	return 0;
}
...
int netsdk_af_callback(int focusMetries, int* param)
{
...
int APP_NETSDK_http_init()
{
	...
	for(i = 0; i < n_ptz_ch; ++i){
		snprintf(uri_fmt, sizeof(uri_fmt), netsdk_uri(uri,"PTZ","channel","%d",NULL), i + 1);
		WEBS_add_cgi(uri_fmt, NETSDK_http_service, kH_METH_ALL);
		snprintf(uri_fmt, sizeof(uri_fmt), netsdk_uri(uri,"PTZ","channel","%d","properties",NULL), i + 1);
		WEBS_add_cgi(uri_fmt, NETSDK_http_service, kH_METH_ALL);
		snprintf(uri_fmt, sizeof(uri_fmt), netsdk_uri(uri,"PTZ","channel","%d","control",NULL), i + 1);
		WEBS_add_cgi(uri_fmt, NETSDK_http_service, kH_METH_ALL);
	}
