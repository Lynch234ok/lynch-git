...
#define PLAY_FALSE_NUM_MAX  2       // ≤•∑≈¡¨Ω” ß∞‹◊Ó¥Û¥Œ ˝
	...
	if(APP_WIFI_model_exist() && wlan.wireless.wirelessMode == NSDK_NETWORK_WIRELESS_MODE_STATIONMODE){
        ...
        if(1 == ret && (link_info.linkstatus == 1 && link_info.sinal_strength >= 10)){
            ...
            /*
                »Áπ˚WIFI_CONNECT_FLAGŒƒº˛¥Ê‘⁄‘Ú±Ì√˜…Ë±∏“—≈‰÷√≤¢≥…π¶¡¨Ω”π˝¬∑”… 
                À˘“‘≤ª≤•∑≈Ã· æ“Ù£¨“≤≤ª◊ˆ20√Î≈–∂œ¡¨Ω” ß∞‹∫Û«–ªªµΩapƒ£ Ω
                **∫Û√Ê”–ø…ƒ‹ª·‘ˆº”∆‰À¸¡¨Ω” ß∞‹µƒ¬ﬂº≠
            */
            if(IS_FILE_EXIST(WIFI_CONNECT_FLAG)) {
                play_false_num = 0;
                return;
...
//#if defined(WIFI)
    if(strcmp(ether_iface, "eth0") != 0) {
        // ‰∫ßÊµãÈÖçÁΩÆÊñá‰ª∂ËøûÊé•Ë∑ØÁî±ÊµãËØïÂπ∂‰∏îÊòØÊ≤°ÊèíÁΩëÁ∫øÊÉÖÂÜµ‰∏ãÔºåÂõ†‰∏∫ipÊ≤°‰øùÂ≠òÂà∞ÈÖçÁΩÆÊñá‰ª∂ÔºåÊâÄ‰ª•‰ºöÂØºËá¥hichip‰∏çÊñ≠ÈáçÂêØ
...
#if defined(MSG)
    TICKER_add_task(ESEE_msg_push_enable, 1, false);
#endif
