...
static int prosess_sendpack(OwspSession_t* session, OWSP_CONX owsp)
{
	...
	if(!session->is_init && session->user == NULL) {
		...
		for(i = 0;; ++i){
			char media_name[32] = {""};
			int lookup_ch = 0;
			snprintf(media_name, sizeof(media_name), "ch%d_%d.264", 0, i); // FIXME:
			lookup_ch = MEDIABUF_lookup_byname(media_name);
			if(lookup_ch < 0){
				break;
			}
			media_ch = lookup_ch;
		...
		mediabuf_in_speed = MEDIABUF_in_speed(media_ch);
		if(mediabuf_in_speed > 0){
			session->user = MEDIABUF_attach(media_ch);
			//OWSP_TRACE("session->user = %08x", session->user);
		}
		OWSP_TRACE("Lookup ch %d for OWSP", media_ch);
	}
	if(session->is_login){
		if(session->user){
